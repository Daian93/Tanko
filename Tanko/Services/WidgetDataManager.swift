...
final class WidgetDataManager {
    ...
    func save(_ mangas: [ReadingManga]) {
        ...
        do {
            ...
            print(
                "📝 Contenido: \(String(data: data, encoding: .utf8) ?? "no decodificable")"
            )
    ...
    func load() -> ReadingCollection {
        ...
        guard let data = try? Data(contentsOf: fileURL) else {
            print(
                "⚠️ Widget: No se pudo leer el archivo, devolviendo colección vacía"
            )
            return .empty
        }
        ...
        for manga in decoded.mangas {
            print(
                "  📚 \(manga.title) - \(manga.coverURL?.absoluteString ?? "sin URL")"
            )
        }
